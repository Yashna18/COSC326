/*
 * This following program takes in an int of a arbitrarily large size
 * and overloads operators in order to work with them. 
 * Compliments the rational file
...
namespace cosc326 {
	...
	//Assigns the value of i to current Integer
	...
	//Overloading operator -
	//Negates the sign of the current Integer
	...
	//Overloading operator +
	//This function takes in two Integer types 
	...
	//Overloading operator -
	//This takes in two numbers of type Integer
	//and returns a new Integer with the result 
	...
	//Overloading operator *
	//This function takes in two numbers of type Integer
	//and returns a new Integer containing the result of the operation
	...
	Integer operator*(const Integer& lhs, const Integer& rhs) {
		...
		result.sign = true; //assign results default sign as positive and check for any cases where this will change
		if(!rhsCopy.sign && lhsCopy.sign) { //if rhs is negative and lhs is positive, the result will be negative
			result.sign = false;
		}
		else if (rhsCopy.sign && !lhsCopy.sign) { //if lhs is negative and rhs is positive, the result will be negative.
			result.sign = false;
		}

        if (lhsCopy == BIGINTEGER_ZERO || rhsCopy == BIGINTEGER_ZERO) { //check if either side is 0
            return BIGINTEGER_ZERO;
        }
        if (lhsCopy.numDigits == 1 && lhsCopy.digits[0] == 1) { //check if lhs is 1 and return rhs if it is
            result = rhsCopy;
            return result;
        }
        if (rhsCopy.numDigits == 1 && rhsCopy.digits[0] == 1) { //check if rhs is 1 and return lhs if it is
            result = lhsCopy;
            return result;
        }
		...
		//begin long multiplication
		//algorithm found here:
		//https://www.hindawi.com/journals/jam/2014/107109/alg2/
        ...
        for (int i = 0; i < lhsCopy.numDigits; i++) {
            ...
            for (int j = 0; j < rhsCopy.numDigits; j++) {
                ...
                int add = 0; //handles the add needed for each digit in result

                if (prod.numDigits > 0) { //if there was a result
                    add = carry + prod.digits[0]; //prod[uv] as seen in algorithm
                }
                else {
                    add = carry;
                }

                if (add >= 10) { //checks for overflow
                    carry = 1;
                    add -= 10;
                }
                else {
	...
	//an integer for the remainder in the division
	...
	//Overloading operator <
	//Takes in two Integer type numbers
	...
	//GCD function, not used in this class
	//Used in rationals
