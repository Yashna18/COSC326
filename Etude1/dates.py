        ...
        if (info_source == "q"):
            ...
            while (start_up):
                ...
                while(user_input != "y" and user_input != "q"):
                    user_input = input("Invalid prompt! Please try again: ")
                if (user_input == "y"):
                    main()
                elif (user_input == "q"):
                    start_up = 0
            ...
            info_source = input("This is not a valid option. Please try again from the following options:\nManual Entry: m\nFile: f\nQuit: q\n")
    ...
    if (checked_year != "Invalid"):
        if (checked_month != "Invalid"):
            ...
            if(checked_day != "Invalid"):
                if (len(str(checked_day)) < 2):
                    return "0{} {} {}\nThis is a valid date\n".format(checked_day, list_of_months[month_num-1], checked_year)
                else:
                    return "{} {} {}\nThis is a valid date\n".format(checked_day, list_of_months[month_num-1], checked_year)
            else:
                return "{}\nIncorrect day!\n".format(entered_date)
...
def checkMonth(date_y, valid_months):
    ...
    if (type(month) is int):
        if (month < 1 or month > 12 or length_month > 2):
            return "Invalid"
        else:
            return month
    else:
